...
--  In other words, you need to determine the number of players who logged in
--  on the day immediately following their initial login, and divide it by
...
(
	SELECT  player_id
	       ,MIN(event_date) AS minimo
	FROM Activity
	GROUP BY  player_id
)
SELECT  ROUND((COUNT(DISTINCT a.player_id) / (
SELECT  COUNT(DISTINCT player_id)
FROM menor)), 2) AS fraction
FROM menor m
JOIN Activity a
ON a.player_id = m.player_id AND DATE_ADD(m.minimo, INTERVAL 1 DAY) = a.event_date
