...
"""
def solution(st_roman: str) -> bool:
    """ Convierte numeros romanos a número enteros
    
    Parmeters:
		- st_roman *(str)* - Número romano
	Returns:
		- in_operation *(int)* - Número romano convertido a entero
    """

    di_roman = {"I":1, "V":5, "X":10, "L":50, "C":100, "D":500, "M":1000}
    in_operation = 0

    for jj in range(0,len(st_roman),1):
        if jj < len(st_roman)-1 and di_roman[st_roman[jj]] < di_roman[st_roman[jj+1]]:
            in_operation -= di_roman[st_roman[jj]]
        else:
            in_operation += di_roman[st_roman[jj]]

    return in_operation

prove = ["III","LVIII","MCMXCIV","IX","XV","XXXVIII"]
